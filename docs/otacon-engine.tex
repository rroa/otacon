...
\textbf{Float is the default and fixed-point is a didactic option}, not a design
commitment the engine is built around. Choose \code{FIXED} when you want what it
actually offers --- bit-identical results across machines, for lockstep netcode
or an exactly reproducible replay --- and remember that Q16.16 tops out at
$\pm32767.99998$, which an endless runner's world X exceeds. Building the test
suite both ways does make a useful second opinion on floating-point assumptions,
but that is a bonus rather than the reason the option exists.
...
\code{Resources} is an engine-owned, load-once cache for textures and sounds,
handed to every game through \code{GameContext}. A single owner is the point: two
systems asking for the same file get the same handle, and everything is released
once, after the game shuts down and while the renderer is still alive. Before it
existed the three games carried 23 hand-written \code{loadPng} calls and 34
matching \code{destroyTexture} calls between them; there are now none.
\code{SaveData} is the matching store for small persistent state --- a high
score, a settings flag --- writing atomically and reading totally.
...
Twenty sample screens (\code{src/samples/}, executable \code{samples}) exercise
the engine on its own. The first fifteen are the canonical set; the rest cover
the facilities those do not reach, on the rule that no subsystem ships without
something that exercises it --- a facility nothing calls is indistinguishable
from one that does not work. They link \code{otacon} and nothing else, and generate
