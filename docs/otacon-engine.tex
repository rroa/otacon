...
src/engine/          # ---- Otacon engine (static lib 'otacon') ----
  ...
  asset/             #   In-house PNG decode/encode + CAF audio decode
  audio/             #   IAudio seam, software Mixer, CoreAudio / null backend
  platform/          #   IWindow seam; glfw/ and sdl2/ backends
  render/            #   IRenderer seam; gl_modern/ gl_legacy/ vulkan/
  scene/             #   Entity, Node, Scene, Camera, Collision, Emitter
...
See Chapter~\ref{ch:draw}. A backend implements only triangle submission and the
frame lifecycle; all shapes are tessellated once in the shared \code{IRenderer}
base, so every backend draws identical geometry.
...
freed with \code{destroyTexture}. \textbf{The cross-backend contract:} a backend
only implements \code{submitTriangles} (solid) and \code{submitTextured}, plus
\code{beginFrame}/\code{endFrame}. Every shape above --- rectangles, oriented
lines, outlines, the bitmap font --- is tessellated to triangles once, in the
shared base class, so all backends rasterise byte-for-byte identical geometry.
...
\begin{itemize}[nosep]
  ...
  \item \textbf{Vulkan.} A full implementation: instance (MoltenVK portability),
        ...
        \emph{not} flip $y$ as the GL shaders do. Validation-clean.
\end{itemize}
